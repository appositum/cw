...
fn main() {
    ...
    match matches.get_many::<String>("file") {
        None => {
            ...
            let parsed_input = cw::parse_content(String::new(), input);

            let max_width = if parsed_input.bytes > 0 {
                // `x.ilog10() + 1` == number of digits in `x`
                parsed_input.bytes.ilog10() + 1
            } else {
                1
            };

            cw::print(parsed_input, max_width as usize);
        }
        Some(mut input_files) => {
